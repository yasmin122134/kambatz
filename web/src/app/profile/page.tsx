...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    setFlags({
      km: data.km,
      exam: data.exam,
      no_weapon: data.no_weapon,
      no_guard: data.no_guard,
      no_mag: data.no_mag,
    });
    setLoading(false);
  }, [router]);
  ...
  return (
    <PageShell
      title="הפרופיל שלי"
      lede="עדכנו את הסימונים האישיים שלכם — הם משפיעים על השיבוץ במחולל."
    >
      <div className="card max-w-lg space-y-5">
        ...
        <form onSubmit={save} className="space-y-4">
          <fieldset className="space-y-3">
            <legend className="font-display text-base mb-2">
              סימונים לשיבוץ
            </legend>
            {FLAG_KEYS.map((key) => (
              <label
              ...
              >
                ...
                />
                <span>{PERSONAL_FLAG_LABELS[key]}</span>
              </label>
            ))}
          </fieldset>
